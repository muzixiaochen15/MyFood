...
- (instancetype)initWithImages:(NSArray *)images selectedIndices:(NSIndexSet *)selectedIndices borderColors:(NSArray *)colors {
    if (self = [super init]) {
        ...
        _tintColor = [CommUtls colorWithHexString:@"#31364a" withAlpha:0.73f];        _borderWidth = 2;
        _itemBackgroundColor = [CommUtls colorWithHexString:@"#212636" withAlpha:0.75f];
...
- (void)layoutItems {
    ...
    [self.itemLabels enumerateObjectsUsingBlock:^(UILabel *label, NSUInteger idx, BOOL * stop) {
        CGRect labelFrame = CGRectMake(leftPadding, (topPadding + self.itemSize.height) * (idx + 1), self.itemSize.width, self.itemSize.height);
